...
Caution : Have patience due to lack of resources, it can get sleep due to inactivity; if you are first visitor within 30 minutes, the website can slow down. Wait for few minutes after first hit.
...
-   Validate real demand fast without feature noise.
-   Spend compute only where it protects the core flows.
-   Surface clear signals (logs + throttles) before scaling.
...
2. set .env accordingly, restart `php artisan config:clear`
3. Visit `http://localhost:8080`
4. OTP appears in logs (`MAIL_MAILER=log`). Repeat the verification checklist above.
...
Use the protected seed endpoint to populate default tours in production:

```bash
curl -X POST https://your-app-domain/seed-tour \
  -H "X-Seed-Token: your-seed-token"
```

Requirements:

-   `SEED_TOKEN` must be set in Railway env vars to match the header.
-   `APP_ENV=production`.
...
**Shaon Majumder**  
Senior Platform Engineer → Engineering Manager (CTO Track)  
Platform, Infrastructure & Systems Architecture | AI-Enabled Products

Actively exploring **Platform Engineering, Staff/Principal Engineer, System Architect, and Engineering Leadership** roles.  
Focused on building **reliable, scalable, and developer-friendly platforms** that enable teams to ship faster and safer.

---

### 🧩 Platform Expertise

-   Platform & backend architecture (Laravel, PHP, APIs)
-   Infrastructure-aware application design
-   CI/CD, containerization, and cloud-native workflows
-   Reliability, security, and operational excellence
-   AI platform integration (OpenAI, Gemini, MCP)
...
Healthcare · Fintech · Telecom · Logistics · Garments · E-commerce · Robotics
